// Duplicated in relay-server/src/protocol/serialize.rs; keep in sync.

use crate::protocol::error::ProtocolError;
use crate::protocol::packet::RoomInfo;

// Must match the server, or one side rejects strings the other sends.
pub const MAX_STRING_LEN: usize = 256;

// A RoomInfo is two 4-byte length prefixes plus payload.
const MIN_ROOM_INFO_BYTES: usize = 8;
...
pub fn read_vec_room_info(bytes: &[u8]) -> Result<(Vec<RoomInfo>, &[u8]), ProtocolError> {
    ...
    // Bounds the allocation by datagram size; i32::MAX would otherwise abort the process.
